...
from premode.benchmark import run_benchmark, format_benchmark_report
...
def test_benchmark_runs_prompt_suite_and_reports_metrics(repo):
    ...
    assert case["cacheable_prefix_tokens"] >= 1024
...
def test_benchmark_json_out_and_human_format(repo):
    ...
    assert "case | packet_tokens" in text
...
def test_v266_docs_are_current_and_concise():
    ...
    assert "v0.2.6.11" in index
    assert "v0.2.6.11" in report
