...
def _prepare_rich_cli_repo(repo: Path) -> None:
    init_project(repo)
...
def test_rich_cli_verification_edit_file_is_contract_inside(repo: Path) -> None:
    _prepare_rich_cli_repo(repo)
    compiled = _compile_rich(repo)

    assert compiled["review_contract"]["candidate_edit_files"] == ["src/rich_cli/__main__.py"]
    ...
    (repo / "src" / "rich_cli" / "__main__.py").write_text(
        (repo / "src" / "rich_cli" / "__main__.py").read_text(encoding="utf-8").replace(
            'help="Output theme"',
    ...
    assert "src/rich_cli/__main__.py" in result["changed_candidate_files"]
...
def test_rich_cli_source_only_edit_still_passes(repo: Path) -> None:
    _prepare_rich_cli_repo(repo)
    _compile_rich(repo)
    (repo / "src" / "rich_cli" / "__main__.py").write_text(
        (repo / "src" / "rich_cli" / "__main__.py").read_text(encoding="utf-8").replace(
    ...
    assert result["merge_readiness"] == "pass"
    assert result["changed_candidate_files"] == ["src/rich_cli/__main__.py"]
    assert result["changed_verification_edit_files"] == []


def test_rich_cli_unrelated_file_edit_warns_outside_packet(repo: Path) -> None:
    _prepare_rich_cli_repo(repo)
    _compile_rich(repo)
    (repo / "docs").mkdir()
...
def test_rich_cli_support_file_edit_warns_separately(repo: Path) -> None:
    _prepare_rich_cli_repo(repo)
    _compile_rich(repo)
...
def test_review_contract_normalizes_locator_support_dicts() -> None:
    contract = _review_contract_from_manifest(
        {
            "patch_boundary": {
                "allowed_edit_files": [{"path": "src/rich_cli/__main__.py"}],
            },
            "support_files": [
                {
                    "path": "README.md",
    ...
    assert contract["candidate_edit_files"] == ["src/rich_cli/__main__.py"]
...
def test_rich_cli_prompt_forbidden_edit_still_blocks(repo: Path) -> None:
    _prepare_rich_cli_repo(repo)
