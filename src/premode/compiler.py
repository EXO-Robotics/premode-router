...
def select_context(
    ...
    *,
    ...
    repo_shape_paths = inventory_paths if inventory_paths is not None else [str(entry.get("path") or "") for entry in entries if entry.get("path")]
    repo_shape = _repo_shape_from_inventory(inventory if isinstance(inventory, dict) else None, repo_shape_paths)
    ...
    try:
        if is_media_lookup_prompt(raw_prompt):
            locator_result = locate_media_files(repo_root, raw_prompt, max_files=8, inventory_paths=inventory_paths)
