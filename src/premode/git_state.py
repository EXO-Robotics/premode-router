...
def _run_git(repo_root: Path, args: list[str], *, timeout: int = 5, cap: int = 200000) -> tuple[bool, str]:
    try:
        cp = subprocess.run(["git", *args], cwd=repo_root, capture_output=True, text=True, timeout=timeout, check=False)
...
def scan_git_state(repo_root: Path, max_diff_bytes: int = 200000) -> dict[str, Any]:
