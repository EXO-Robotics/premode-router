...
TOOLS = [
    {
        "name": TOOL_NAME,
        "description": "Transform an exact coding task into bounded, repository-relative pCodex guidance.",
...
def bind_workspace(value: str | Path | None) -> WorkspaceBinding:
    ...
    if not lexical.is_absolute() or ".." in lexical.parts:
        raise WorkspaceBindingError("configured workspace must be an absolute normalized path")
    try:
...
def _validate_arguments(arguments: Any) -> dict[str, Any]:
    ...
    if spawn_metadata is not None:
        if not isinstance(spawn_metadata, dict) or any(not isinstance(key, str) for key in spawn_metadata):
...
def _tool_result_payload(result: Any) -> dict[str, Any]:
    ...
    if isinstance(metadata, dict):
        ...
        ] if isinstance(metadata.get("selected_paths"), list) else []
...
def call_tool(
    ...
    *,
    ...
    if workspace is not None:
        ...
        # Retained only for the in-process compatibility/test API. The stdio
        # server always supplies an immutable WorkspaceBinding.
        project_root = cwd.resolve(strict=True)
    else:
        # Historical in-process callers never represented the registered MCP
        # transport. Production stdio execution cannot reach this fallback.
    ...
    workspace.assert_current() if workspace is not None else None
...
class McpSession:
    ...
    @staticmethod
    def _exact_params(
        params: dict[str, Any], allowed: set[str], required: set[str] | None = None,
    ) -> bool:
    ...
    def _begin(self, request_id: Any) -> str:
        ...
        with self._lock:
            ...
            if self.shutting_down:
                return "cancelled"
            if key in self._in_flight or len(self._in_flight | self._cancelled) >= MAX_CONCURRENT_REQUESTS:
                return "busy"
        ...
        *,
        ...
        if method == "initialize":
            if is_notification or self.initialized or self.shutting_down:
                return None if is_notification else _error(request_id, -32600, "invalid initialize request")
            ...
            if not isinstance(protocol, str) or not isinstance(capabilities, dict) or not isinstance(client_info, dict):
                return _error(request_id, -32602, "invalid initialize params")
            self.initialized = True
            negotiated = protocol if protocol in SUPPORTED_PROTOCOL_VERSIONS else PROTOCOL_VERSION
            return _response(
                request_id,
                result={
                    "protocolVersion": negotiated,
                    "capabilities": {"tools": {"listChanged": False}},
                    "serverInfo": {"name": "pcodex", "version": __version__},
                },
            )
        ...
        if method == "shutdown":
            if is_notification or not self.initialized:
                return None if is_notification else _error(request_id, -32600, "invalid shutdown request")
            if params:
        ...
        try:
            ...
            work = ToolWork(request_id=request_id, name=params["name"], arguments=arguments)
            if defer_tool:
                return work
            result = call_tool(
                work.name,
            ...
            return _error(request_id, -32800, "request cancelled") if cancelled else _error(request_id, code, str(exc))
        except Exception:
            cancelled = self._finish(request_id)
            return _error(request_id, -32800, "request cancelled") if cancelled else _error(request_id, -32603, "internal tool error")
        cancelled = self._finish(request_id)
        return _error(request_id, -32800, "request cancelled") if cancelled else _response(request_id, result=result)

    def handle(self, request: dict[str, Any], *, cwd: Path | None = None) -> dict[str, Any] | None:
...
def _worker_main(
    ...
    try:
        ...
        connection.send({"error": {"code": -32602, "message": "configured workspace identity changed"}})
...
class ProcessWorkerManager:
    ...
    def start(self, work: ToolWork) -> None:
        ...
        process = self._context.Process(
            target=_worker_main,
            args=(child_connection, work, self._workspace, self._compile_runner),
            name="pcodex-mcp-worker",
        )
    ...
    *,
    ...
    if session is None:
        ...
        if request.get("method") != "initialize":
            # Compatibility for the historical in-process helper. Production
            # stdio execution always uses a persistent, strict session.
...
def handle_line(
    ...
    *,
    ...
    return handle_request(request, cwd=cwd, compile_runner=compile_runner, session=session)
...
def serve(
    *,
    ...
    session = McpSession(workspace=binding, compile_runner=compile_runner)
    output_lock = Lock()
    ...
    def write_response(response: dict[str, Any] | None) -> None:
        if response is None:
            return
        with output_lock:
            output_stream.write(json.dumps(response, sort_keys=True, allow_nan=False) + "\n")
            output_stream.flush()
                ...
                if not line.endswith("\n"):
                    while True:
                        ...
                        if not remainder or remainder.endswith("\n"):
                            break
                write_response(_error(None, -32600, "request exceeds the supported input boundary"))
                continue
            if not line.strip():
                continue
            try:
