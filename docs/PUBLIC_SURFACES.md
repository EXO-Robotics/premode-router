...
- `upgrade`: check the frozen predecessor boundary after explicit package replacement, preserve its actual `.pcodex` user state, and reconcile only separately proven receipt-owned or supported legacy-plugin state;
...
`pcodex install` previews and `pcodex install --apply` establishes the per-file lifecycle authority plus a reinstall-validation receipt bound to the exact authority hash. `pcodex upgrade --check` is literal no-write; after explicit installation of the qualified replacement artifact, `pcodex upgrade --apply` preserves the actual predecessor `.pcodex` state and changes only separately receipt-proven compatible state or supported historical plugin fingerprints. It does not invoke a package manager or registry. `pcodex repair --dry-run` is literal no-write; `pcodex repair --yes` restores only a missing receipt-proven generated item whose authoritative product bytes match the installed hash. A missing ownership marker additionally requires the current reinstall-validation proof. Modified or alternate filesystem objects are preserved. Repair uses no-follow directory descriptors, private staging, no-replace commit for missing leaves, file fsync, atomic authority replacement, best-effort parent fsync, and rollback if receipt completion fails.
